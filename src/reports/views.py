...
# working with multiple forms within one view
@login_required
def report_view(request, production_line):
	form= ReportForm(request.POST or None, production_line=production_line)
	pform=ProblemReportedForm(request.POST or None)# prefilling the form
	queryset=Report.objects.filter(production_line__name=production_line)
	line= get_object_or_404(ProductionLine, name=production_line) #accessing ProductionLine with production_line from request
	...
	if 'submitbtn1' in request.POST:
		...
		if pform.is_valid():
			...
			obj.report=report
			obj.save()        #saving the form data to the database
